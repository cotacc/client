...
const SecretaryReport = () => {
  ...
  const handleDate = async (month, year) => {
    try {
      ...
      const { receivememo, sentmemo } = response.data;
      setMonthlyData(generateMonthlyData(receivememo, sentmemo));
    } catch (error) {
      console.error(error);
      setMonthlyData([]);
    }
  ...
  const generateMonthlyData = (receiveMemos, sendMemos) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      const sentOnDay = sendMemos.filter(memo => new Date(memo.createdAt).getDate() === day);
      ...
      monthlyData.push({
        ...
        sentMemos: sortMemosByTitle(sentOnDay),
          ...
          <div className="report-list">
            ...
            <div className="report-list-memo-details">
              ...
              <div className="memo-details-on-that-day">
                {monthlyData.filter(dayData => dayData.Sent > 0 || dayData.Received > 0).length > 0 ? (
                  monthlyData.filter(dayData => dayData.Sent > 0 || dayData.Received > 0).map((dayData, index) => (
                      ...
                      <div className="memo-details-list">
                        {dayData.sentMemos.length > 0 && (
                          <div className="memo-section">
                            <h4 className="section-title">Sent Memos</h4>
                            <ul>
                              {dayData.sentMemos.map((sentMemo, sentIndex) => (
                                <li key={`sent-${sentIndex}`} className="memo-item">
                                  <b>Title:</b> {sentMemo.title}{' '}
                                  <Link to={`/secretary/sent_report/${sentMemo._id}`} className="view-details">
                                    View Details
                                  </Link>
                                </li>
                              ))}
                            ...
                            <ul>
                              {dayData.receivedMemos.map((receivedMemo, receivedIndex) => (
                                <li key={`received-${receivedIndex}`} className="memo-item">
                                  ...
                                  <Link to={`/secretary/receive_memo/${receivedMemo._id}`} className="view-details">
                                    View Details
                                  </Link>
                                </li>
                              ))}
